...
`.claude/rules/response-footer.md` — the block that closes **every** reply: where
things stand, what happens next, which agents are on it, and what needs a human.
It is loaded automatically at session start; read it if you are unsure of the format.
